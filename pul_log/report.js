...
async function loadAndRender() {
  ...
  const kcalGoal = 2000; // 서버에서 목표 칼로리를 내려주지 않는 경우 기본 2000
  ...
  // 2) 도넛 하단 칼로리 요약(미니 링 자리 대체)
  const inlineNow  = document.getElementById("kcalInlineNow");
  const inlineGoal = document.getElementById("kcalInlineGoal");
  if (inlineNow)  inlineNow.textContent  = toComma(kcalNow);
  if (inlineGoal) inlineGoal.textContent = toComma(kcalGoal);

  // 3) (삭제됨) 작은 원형 게이지 → 하단 텍스트로 대체
  //    미니 링 관련 DOM/스타일/함수는 제거했습니다.
